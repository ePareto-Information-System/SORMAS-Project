...
public final class CaseLogic {
	 ...
	 * 
	 * @param caze       The new CaseDataDto for which the facility change should be
	 *                   handled.
	 * @param oldCase    The Dto of the existing case being changed.
	 * @param isTransfer Indicates if the old case is transferred (both from or to a
	 *                   hospital).
	 */
	public static void handleHospitalization(CaseDataDto caze, CaseDataDto oldCase, boolean isTransfer) {
		// todo (@JonasCir) I feel this whole class or at least this method should be
		// absorbed by the case EJB
		// case is already in a hospital and is transferred from it (discharge or other
		// hospital)...
		...
		if (isTransfer || !FacilityType.HOSPITAL.equals(caze.getFacilityType())) {
			...
			try {
				PropertyDescriptor[] pds = Introspector.getBeanInfo(HospitalizationDto.class, EntityDto.class)
						.getPropertyDescriptors();

				for (PropertyDescriptor pd : pds) {
					// Skip properties without a read or write method
					if (pd.getWriteMethod() == null || HospitalizationDto.HOSPITALIZED_PREVIOUSLY.equals(pd.getName())
	...
	/**
	 * Calculates the follow-up until date of the case based on its start date
	 * (onset contact or report date), the follow-up duration of the disease, the
	 * current follow-up until date and the date of the last cooperative visit.
	 *
	 * @param ignoreOverwrite Returns the expected follow-up until date based on
	 *                        case start date, follow-up duration of the disease and
	 *                        date of the last cooperative visit. Ignores current
	 *                        follow-up until date and whether or not follow-up
	 *                        until has been overwritten.
	 */
	public static Date calculateFollowUpUntilDate(CaseDataDto caze, List<VisitDto> visits, int followUpDuration,
		...
		Date untilDate = !ignoreOverwrite && caze.isOverwriteFollowUpUntil() ? caze.getFollowUpUntil()
				: standardUntilDate;

		Date lastVisitDate = null;
		boolean additionalVisitNeeded = true;
		for (VisitDto visit : visits) {
			if (lastVisitDate == null || DateHelper.getStartOfDay(visit.getVisitDateTime())
					.after(DateHelper.getStartOfDay(lastVisitDate))) {
		...
		// Follow-up until needs to be extended to the date after the last visit if
		// there is no cooperative visit after the follow-up until date
		...
		// If the follow-up until date is before the standard follow-up until date for
		// some reason (e.g. because the report date and/or last contact
