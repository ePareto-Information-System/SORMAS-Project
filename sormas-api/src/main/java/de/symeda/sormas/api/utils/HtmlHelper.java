...
public class HtmlHelper {
	...
	public static final Whitelist EVENTACTION_WHITELIST = Whitelist.relaxed().addTags("hr", "font")
			.addAttributes("font", "size", "face", "color").addAttributes("div", "align");
	...
	/**
	 * @param attributeKey   For example {@code title}.
	 * @param attributeValue The value for the given {@code attributeKey}.
	 * @return Built syntax for a HTML tag attribute with possible html tags in
	 *         {@code attributeValue} escaped to prevent HTML injection.
	 */
	...
	// this method should be used for i18n-strings and captions so that custom
	// whitelist rules can be added when needed
	...
	/**
	 * @param title   Title for {@code a} tag (hover text).
	 * @param caption Caption of the {@code a} tag (visual text).
	 * @return Generated hyperlink with possible html tags escaped to prevent HTML
	 *         injection.
	 */
	...
	private static String buildTitle(String tag, String title, String caption) {
		...
		String result = String.format("<%s %s>%s</%s>", tag, cleanHtmlAttribute(TITLE_ATTRIBUTE, title),
				HtmlHelper.cleanHtml(caption), tag);
