...
public class PersonDto extends PseudonymizableDto {
	...
	@Diseases({ Disease.AFP, Disease.EVD, Disease.GUINEA_WORM, Disease.LASSA, Disease.POLIO, Disease.CORONAVIRUS,
	...
	private void setPersonContactInformation(String contactInfo, PersonContactDetailType personContactDetailType) {
		for (PersonContactDetailDto contactDetailDto : getPersonContactDetails()) {
			if (contactDetailDto.getPersonContactDetailType() == personContactDetailType
					&& contactDetailDto.isPrimaryContact()) {
				contactDetailDto.setPrimaryContact(false);
			}
		}
		final PersonContactDetailDto pcd = PersonContactDetailDto.build(this.toReference(), true,
				personContactDetailType, null, null, contactInfo, null, false, null, null);
		getPersonContactDetails().add(pcd);
	}

	/**
	 *
	 * @return the String representation of the PRIMARY phone number. Phone numbers
	 *         set with the {@link #setPhone(String)} method automatically become
	 *         primary. A phone number entered in the personEditForm is not, and
	 *         thus does not become primary phone number unless the user
	...
	/**
	 *
	 * @param onlyPrimary if true, the return value is same as in
	 *                    {@link #getPhone()}. Otherwise, this method tries to
	 *                    return the only phone number for this person, no matter if
	 *                    primary or not. Results in an
	 *                    SeveralNonPrimaryContactDetailsException when there are
	...
	public String getPhone(boolean onlyPrimary) throws SeveralNonPrimaryContactDetailsException {
		...
		if (onlyPrimary || !(primaryPhone.equals(""))) {
			...
			if (allPhones.size() == 0) {
				...
				throw new SeveralNonPrimaryContactDetailsException(
						"Too many results found, none of which is marked primary.");
			} else {
	...
	/**
	 * 
	 * @param phone is automatically set as primary phone number, removing the
	 *              primary status from another phone number if necessary.
	 */
	...
	/**
	 * 
	 * @return the PRIMARY email address. Email addresses set with the
	 *         {@link #setEmailAddress(String)} method automatically become primary.
	 *         An email address entered in the personEditForm is not, and thus does
	 *         not become primary email address unless the user specifically sets
	 *         it.
	 */
	@JsonProperty(access = JsonProperty.Access.READ_ONLY)
	 ...
	 * 
	 * @param onlyPrimary if true, the return value is same as in
	 *                    {@link #getEmailAddress()}. Otherwise, this method tries
	 *                    to return the only email address for this person, no
	 *                    matter if primary or not. Results in an
	 *                    SeveralNonPrimaryContactDetailsException when there are
	...
	/**
	 * 
	 * @param email is automatically set as primary email address, removing the
	 *              primary status from another email address if necessary.
	 */
	...
	public boolean isEnrolledInExternalJournal() {
		return SymptomJournalStatus.ACCEPTED.equals(symptomJournalStatus)
				|| SymptomJournalStatus.REGISTERED.equals(symptomJournalStatus);
	}
