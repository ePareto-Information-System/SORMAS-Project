...
public class EbsController {
	...
	public CommitDiscardWrapperComponent<TriagingDataForm> getEbsCreateTriagingComponent(final String ebsUuid, boolean isEditAllowed) {
		EbsDto ebs = findEbs(ebsUuid);
		TriagingDataForm triagingDataForm = new TriagingDataForm(ebs, EbsDto.class, ebs.isPseudonymized(), true, isEditAllowed);
	...
	public CommitDiscardWrapperComponent<SignalVerificationDataForm> getEbsCreateSignalVerficationComponent(
		...
		EbsDto ebs = findEbs(ebsUuid);
		SignalVerificationDataForm signalVerificationDataForm =
			new SignalVerificationDataForm(ebs, EbsDto.class, ebs.isPseudonymized(), true, isEditAllowed);
	...
	public CommitDiscardWrapperComponent<RiskAssessmentDataForm> getEbsCreateRiskAssessmentComponent(final String ebsUuid, boolean isEditAllowed) {
		...
		RiskAssessmentDataForm riskAssessmentDataForm = new RiskAssessmentDataForm(ebs, EbsDto.class, ebs.isPseudonymized(), true, isEditAllowed);
	...
	public void showAssessmentCaseDialog(RiskAssessmentDto riskAssessmentDto) {
		...
		Label recommendedLabel = new Label(String.format(I18nProperties.getString(Strings.RecommendedActions), 7, 7), ContentMode.HTML);
		recommendedLabel.addStyleName(CssStyles.H1);
		Label recommended = null;
		Label notificationTypeLabel = new Label(String.format(I18nProperties.getString(Strings.NotificationType), 7, 7), ContentMode.HTML);
		...
		if (riskAssessmentDto.getRiskAssessment() == RiskAssesment.VERY_HIGH) {
			...
			notificaticationAction =
				new Label(String.format(I18nProperties.getString(Strings.VeryHighRiskNotificationAction), 50, 50), ContentMode.HTML);
			...
			verticalLayout.addComponents(
				...
				notificaticationAction);
			Window window = VaadinUiUtil.showPopupWindow(verticalLayout, I18nProperties.getCaption(riskCaption.getValue()));
			...
			notificationType.removeStyleName("v-label-undef-w");
			notificaticationAction =
				new Label(String.format(I18nProperties.getString(Strings.ModerateRiskNotificationAction), 50, 50), ContentMode.HTML);
			notificaticationAction.setStyleName("risk-label");
			notificaticationAction.removeStyleName("v-label-undef-w");
			VerticalLayout verticalLayout = new VerticalLayout();
			verticalLayout.setWidth(80, Sizeable.Unit.PERCENTAGE);
	...
	public CommitDiscardWrapperComponent<EbsAlertDataForm> getEbsCreateAlertComponent(final String ebsUuid, boolean isEditAllowed) {
		EbsDto ebs = findEbs(ebsUuid);
		EbsAlertDto ebsAlertDto = EbsAlertDto.build(ebs.toReference());
		EbsAlertDataForm alertDataForm = new EbsAlertDataForm(ebs, EbsDto.class, ebs.isPseudonymized(), true, isEditAllowed);
...
//		}
	...
	public CommitDiscardWrapperComponent<EbsAlertDataForm> getEbsCreateAlertEditComponent(
		final String ebsUuid,
		boolean isEditAllowed,
		EbsAlertDto ebsAlertDto) {
		EbsDto ebs = findEbs(ebsUuid);
		EbsAlertDataForm alertDataForm = new EbsAlertDataForm(ebs, EbsDto.class, ebs.isPseudonymized(), true, isEditAllowed);
		alertDataForm.setValue(ebsAlertDto);
		final CommitDiscardWrapperComponent<EbsAlertDataForm> editView =
	...
	public boolean isTriagingDiscard(String ebsUuid) {
		...
		return ebs.getTriaging().getTriagingDecision() == EbsTriagingDecision.DISCARD
			|| ebs.getTriaging().getTriagingDecision() == EbsTriagingDecision.MORE_INFORMATION;
	}

	public RiskAssessmentDto createRiskAssessmentComponent(final String ebsUuid, boolean isEditAllowed) {
		CommitDiscardWrapperComponent<RiskAssessmentDataForm> createRiskAssessmentComponent =
			getEbsCreateRiskAssessmentComponent(ebsUuid, isEditAllowed);
	...
	public RiskAssessmentDto editRiskAssessmentComponent(final String ebsUuid, boolean isEditAllowed, RiskAssessmentDto riskAssessmentDto) {
		CommitDiscardWrapperComponent<RiskAssessmentDataForm> createRiskAssessmentComponent =
			getRiskAssessmenteditComponent(ebsUuid, isEditAllowed, riskAssessmentDto);
		if (isSignalVerified(ebsUuid)) {
			return null;
		}
		VaadinUiUtil.showModalPopupWindow(createRiskAssessmentComponent, I18nProperties.getString(Strings.headingCreateNewAssessment));
	...
	public EbsAlertDto editAlertComponent(final String ebsUuid, boolean isEditAllowed, EbsAlertDto ebsAlertDto) {
		CommitDiscardWrapperComponent<EbsAlertDataForm> createAlertComponent = getEbsCreateAlertEditComponent(ebsUuid, isEditAllowed, ebsAlertDto);
		if (isSignalVerified(ebsUuid)) {
			return null;
		}
	...
	public EbsAlertDto createAlertComponent(final String ebsUuid, boolean isEditAllowed) {
