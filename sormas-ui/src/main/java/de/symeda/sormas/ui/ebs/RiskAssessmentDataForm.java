...
public class RiskAssessmentDataForm extends AbstractEditForm<RiskAssessmentDto> {
	...
	private static final String HTML_LAYOUT = loc(RISK_ASSESSMENT_LOC)
	...
	protected void addFields() {
		...
		ValueChangeListener commonListener = event -> {
			...
			if (morbidityMortality.getNullableValue() == YesNo.YES
				&& spreadProbability.getNullableValue() == YesNo.YES
				&& controlMeasures.getNullableValue() == YesNo.NO) {
				riskAssesment.setValue(RiskAssesment.VERY_HIGH);
				riskAssesment.setStyleName("very-high-risk-assessment");
			} else if (morbidityMortality.getNullableValue() == YesNo.YES
				&& spreadProbability.getNullableValue() == YesNo.YES
				&& controlMeasures.getNullableValue() == YesNo.YES
				|| morbidityMortality.getNullableValue() == YesNo.NO
					&& spreadProbability.getNullableValue() == YesNo.YES
					&& controlMeasures.getNullableValue() == YesNo.NO
				|| morbidityMortality.getNullableValue() == YesNo.YES
					&& spreadProbability.getNullableValue() == YesNo.NO
					&& controlMeasures.getNullableValue() == YesNo.NO) {
				riskAssesment.setValue(RiskAssesment.HIGH);
				riskAssesment.setStyleName("high-risk-assessment");
			} else if (morbidityMortality.getNullableValue() == YesNo.NO
				&& spreadProbability.getNullableValue() == YesNo.NO
				&& controlMeasures.getNullableValue() == YesNo.NO
				|| morbidityMortality.getNullableValue() == YesNo.YES
					&& spreadProbability.getNullableValue() == YesNo.NO
