...
public class AggregateReportsFilterForm extends AbstractFilterForm<AggregateReportCriteria> {
		...
		if (userRegion != null) {
			...
			if (userDistrict != null) {
				...
				Optional.ofNullable(facilityFilter)
					.ifPresent(f -> f.addItems(FacadeProvider.getFacilityFacade().getActiveHospitalsByDistrict(userDistrict, false, false, true)));
		...
		case AggregateReportCriteria.DISTRICT: {
			...
			if (newDistrict != null) {
				...
				if (facilityFilter != null) {
					FieldHelper.updateItems(facilityFilter, FacadeProvider.getFacilityFacade().getActiveHospitalsByDistrict(newDistrict, false, false, true));
				}
	...
	protected void applyDependenciesOnNewValue(AggregateReportCriteria criteria) {
		...
		if (criteria.getDistrict() != null) {
			if (facilityFilter != null) {
				FieldHelper
					.updateItems(facilityFilter, FacadeProvider.getFacilityFacade().getActiveHospitalsByDistrict(criteria.getDistrict(), false, false, true));
			}
			if (pointOfEntryFilter != null) {
