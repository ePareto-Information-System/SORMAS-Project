...
public class TriagingDataForm extends AbstractEditForm<TriagingDto> {
	...
	protected void addFields() {
		...
		outcomeSupervisor = addField(TriagingDto.OUTCOME_SUPERVISOR, ComboBox.class);
		initFields();
		...
		signalCategory.addValueChangeListener(e -> {
			final Set<String> validCategories = new HashSet<>(Arrays.asList("[Human]", "[Environment]", "[Animal]", "[POE]"));
	...
	public static void reviewSignal(String captionsText) {
		...
		Window window = VaadinUiUtil.showPopupWindow(verticalLayout, I18nProperties.getCaption(Captions.signalReview));
	...
	private void handleNoCase(EbsDto selectedEbs) {
