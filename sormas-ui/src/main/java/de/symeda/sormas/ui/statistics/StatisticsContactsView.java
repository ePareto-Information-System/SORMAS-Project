...
public class StatisticsContactsView extends AbstractStatisticsView {
	...
	private void addGenerateButton(VerticalLayout statisticsLayout) {
		Button generateButton = ButtonHelper.createButton(Captions.actionGenerate, e -> {
			...
			if (showContactIncidence && visualizationComponent.hasAgeGroupGroupingWithoutPopulationData()
				|| hasUnsupportedPopulationAgeGroupFilter()) {
				errorNotification =
					new Notification(I18nProperties.getString(Strings.messageContactIncidenceUnsupportedAgeGroup), Type.ERROR_MESSAGE);
			}
	...
	public void generateMap() {
		...
		if (!showContactIncidence || !contactIncidencePossible) {
			...
			valuesMedian =
				resultData.size() > 0 ? new BigDecimal(resultData.get((int) (resultData.size() * 0.5)).getContactCount()) : BigDecimal.ZERO;
			valuesUpperQuartile =
	...
	private List<StatisticsContactCountDto> generateStatistics() {
		...
		if (showContactIncidence) {
			...
			populationReferenceYear =
				calculateMaximumReferenceYear(null, contactCriteria.getOnsetYears(), Comparator.naturalOrder(), e -> e.getValue());
