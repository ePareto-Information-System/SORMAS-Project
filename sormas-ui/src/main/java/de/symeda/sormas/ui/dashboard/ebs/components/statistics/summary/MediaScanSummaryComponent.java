...
public class MediaScanSummaryComponent extends EbsEventSectionStatisticsComponent {

	private final EbsEventSummaryElementComponent ebsEventSummaryElementComponent;

	public MediaScanSummaryComponent() {
		super(Captions.dashboardMediaScanning, null);
	...
	public void update(DashboardDataProvider dashboardDataProvider) {
		dashboardDataProvider.setEventSourceType(EbsSourceType.MEDIA_NEWS);
		updateTotalLabel(String.valueOf(dashboardDataProvider.getSourceTypeCount().getOrDefault(EbsSourceType.MEDIA_NEWS, 0)));

		List<EbsCategoryOfInformantDto> ebsCategoryOfInformantDtoList = dashboardDataProvider.getEbsCategoryOfInformantDtoBySourceTypeForMediaScan();
