...
public class EbsEventTileComponent extends VerticalLayout {

	private static final long serialVersionUID = 6582975657305031105L;
	private Map<EbsEvent, HorizontalLayout> layoutMap = new LinkedHashMap<>();

	public EbsEventTileComponent(EbsEventBurdenDto ebsEventBurden, DashboardDataProvider dashboardDataProvider) {
		setMargin(false);
		setSpacing(false);
		//ebsEventBurden.getOutbreakDistrictCount() > 0
		addTopLayout(ebsEventBurden.getEbsEvent(), ebsEventBurden.getEbsEventCount(), ebsEventBurden.getPreviousEbsEventCount(),
				false);
//		addStatsLayout(
//			ebsEventBurden.getCaseDeathCount(),
//			ebsEventBurden.getEventCount(),
//			ebsEventBurden.getLastReportedDistrictName(),
//			ebsEventBurden.getEbsEvent());

		
		addStatsLayout(ebsEventBurden, dashboardDataProvider);

	}

	private void addTopLayout(EbsEvent ebsEvent, Long ebsEventCount, Long previousEbsEventCount, boolean isOutbreak) {
		HorizontalLayout layout = new HorizontalLayout();
		layout.setMargin(false);
		layout.setSpacing(false);
		layout.addStyleName(CssStyles.getEbsEventColor(ebsEvent));
		...
		Label nameLabel = new Label(I18nProperties.getEnumCaption(ebsEvent));
		...
		countLayout.addStyleNames(CssStyles.getEbsEventColor(ebsEvent), CssStyles.BACKGROUND_DARKER);
		countLayout.setHeight(100, Unit.PERCENTAGE);
		countLayout.setWidth(100, Unit.PERCENTAGE);

		Label countLabel = new Label(ebsEventCount.toString());
		...
		HorizontalLayout comparisonLayout = new HorizontalLayout();
		{
			...
			String chevronType;
			if (previousEbsEventCount < ebsEventCount) {
				chevronType = VaadinIcons.CHEVRON_UP.getHtml();
			} else if (previousEbsEventCount > ebsEventCount) {
			...
			Label previousCountLabel = new Label(previousEbsEventCount.toString());
	...
	private void addStatsLayout(EbsEventBurdenDto ebsEventBurden,DashboardDataProvider dashboardDataProvider) {
		...
		Date dateTo = ebsEventBurden.getTo();
		...
		StatsItem lastReportItem = new StatsItem.Builder(Captions.EbsEventBurdenLastReportedDistrictName,
                district).singleColumn(true).build();
		...
		Button component = addEbsEventButton(ebsEvent, dashboardDataProvider);
	...
	private Button addEbsEventButton(EbsEvent ebsEventName, DashboardDataProvider dashboardDataProvider) {
		...
		Button ebsEventDetailButton = ButtonHelper.createIconButton(null, VaadinIcons.ELLIPSIS_DOTS_H, null,
				ValoTheme.BUTTON_BORDERLESS, CssStyles.VSPACE_TOP_NONE, CssStyles.VSPACE_4);
		ebsEventDetailButton.setVisible(true);
//		ebsEventDetailButton.addClickListener(
//				click -> ControllerProvider.getDashboardController().navigateToEbsEvent(ebsEventName, from,to,type));

		ebsEventDetailButton.addClickListener(
				click -> ControllerProvider.getDashboardController().navigateToEbsEvent(ebsEventName, dashboardDataProvider));

		
		
//		Button ebsEventDetailButton = ButtonHelper
...
//		ebsEventDetailButton.setVisible(true);
//		layout.addComponent(ebsEventDetailButton);
//		ebsEventDetailButton.addClickListener(click -> ControllerProvider.getDashboardController().navigateToEbsEvent(ebsEvent.getName()));

		return ebsEventDetailButton;
	}
}
