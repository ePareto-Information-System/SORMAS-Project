...
public class EbsEditFragment extends BaseEditFragment<FragmentEbsEditLayoutBinding, Ebs, Ebs> {
	...
	protected void onAfterLayoutBinding(FragmentEbsEditLayoutBinding contentBinding) {
		...
		contentBinding.pickGpsCoordinates.setOnClickListener(v -> {
			...
			confirmationDialog.setPositiveCallback(() -> {
				...
				if (phoneLocation != null) {
					...
					NotificationHelper
						.showDialogNotification((NotificationContext) getContext(), NotificationType.WARNING, R.string.message_gps_problem);
				}
			});
		...
		contentBinding.ebsSourceInformation.addValueChangedListener(e -> {
			...
			if (value == null || !(value instanceof EbsSourceType)) {
				contentBinding.ebsInformantName.setVisibility(GONE);
				return;
			}
			...
			List<Item> filteredInformant =
				categoryOfInformant.stream().filter(item -> itemsToAdd.toString().contains(item.toString())).collect(Collectors.toList());
		...
		contentBinding.ebsCategoryOfInformant.addValueChangedListener(e -> {
			...
			List<Item> filteredInformant =
				mediaScanningType.stream().filter(item -> itemsToAdd.toString().contains(item.toString())).collect(Collectors.toList());

			contentBinding.ebsScanningType.initializeSpinner(filteredInformant);
		});
		contentBinding.ebsReportDateTime.addValueChangedListener(e -> {
			validateDateFields(contentBinding);
		});
		contentBinding.ebsDateOnset.addValueChangedListener(e -> {
			validateDateFields(contentBinding);
		});
	}
				...
				dateOfReport.setValidationCallback(() -> {
					dateOfReport.enableErrorState(
						I18nProperties.getValidationError(
							THE_DATE_OF_REPORT_CANNOT_BE_EARLIER_THAN_THE_DATE_OF_OCCURRENCE,
							THE_DATE_OF_REPORT_CANNOT_BE_EARLIER_THAN_THE_DATE_OF_OCCURRENCE));
					return true;
				});
			} else {
