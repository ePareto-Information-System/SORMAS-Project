	...
	private QueryBuilder<Ebs, Long> buildQueryBuilder(EbsCriteria criteria) throws SQLException {
		...
		if (criteria.getSignalOutcome() != null) {
			if (criteria.getSignalOutcome() == SignalOutcome.EVENT) {
				...
				signalVerificationLongQueryBuilder.where()
					.eq(SignalVerification.VERIFIED, SignalOutcome.NON_EVENT)
					.or()
		...
		if (criteria.getReportDateTime() != null) {
			whereStatements.add(
				where.between(
					Ebs.REPORT_DATE_TIME,
					DateHelper.getStartOfDay(criteria.getReportDateTime()),
		...
		if (criteria.getTriageDate() != null) {
			triagingLongQueryBuilder.where()
				.between(
					Triaging.DATE_OF_DECISION,
					DateHelper.getStartOfDay(criteria.getTriageDate()),
