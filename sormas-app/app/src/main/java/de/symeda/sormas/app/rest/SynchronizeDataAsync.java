...
public class SynchronizeDataAsync extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... params) {
        ...
        if (ConfigProvider.isRepullNeeded()) {
            ...
            Log.d("Sync mode is: ", String.valueOf(syncMode));
        }
        ...
        try {
            ...
            switch (syncMode) {
                case Changes:
                    ...
                    pullInfrastructure();
                    // we must keep the current field sample id updated on the device
                    pullAndRemoveDeletedFieldSampleIds();
...
//                || DatabaseHelper.getFieldSampleIdtDao().isAnyModified();
    }
...
//        List<String> fieldSampleIds = executeUuidCall(RetroProvider.getSampleFacade().getAllFieldSampleIds());
//        Log.d("Check contents: ", String.valueOf(fieldSampleIds));
//        DatabaseHelper.getFieldSampleIdtDao().deleteInvalid(fieldSampleIds);
    ...
    @AddTrace(name = "pullAndRemoveDeletedFieldSampleIds")
    private void pullAndRemoveDeletedFieldSampleIds() throws NoConnectionException, ServerConnectionException, ServerCommunicationException {
        Log.d(SynchronizeDataAsync.class.getSimpleName(), "pullDeletedFieldSampleIds");

            // FieldSampleId
            List<String> fieldSampleIds = executeUuidCall(RetroProvider.getSampleFacade().getAllFieldSampleIds());
            Log.d("Pulling Check contents: ", String.valueOf(fieldSampleIds));
            for (String fieldSampleId : fieldSampleIds) {
                DatabaseHelper.getFieldSampleIdtDao().saveFieldSampleId(fieldSampleId);
//                DatabaseHelper.getFieldSampleIdtDao().deleteFieldSampleIdAndAllDependingEntities(fieldSampleId);
            }
