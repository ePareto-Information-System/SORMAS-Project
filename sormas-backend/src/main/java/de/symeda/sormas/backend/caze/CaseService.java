...
//		System.out.println("Turnaround time2 starting:_____ " + startTime2 + " milliseconds");
	...
	public List<CaseIndexExportDto[]> getCasesForDuplicateMergingExport(CaseCriteria criteria, boolean ignoreRegion, double nameSimilarityThreshold, int start, int max) {
...
//		);
		...
		List<CaseIndexExportDto[]> resultList = new ArrayList<>();
		...
		if (!foundIds.isEmpty()) {
			...
			for (Object[] idPair : foundIds) {
				try {
					// Cloning is necessary here to allow us to add the same CaseIndexDto to the grid multiple times
					CaseIndexExportDto parent = (CaseIndexExportDto) indexCases.get(idPair[0]).clone();
					CaseIndexExportDto child = (CaseIndexExportDto) indexCases.get(idPair[1]).clone();

					if (parent.getCompleteness() == null && child.getCompleteness() == null
							|| parent.getCompleteness() != null
							&& (child.getCompleteness() == null || (parent.getCompleteness() >= child.getCompleteness()))) {
						resultList.add(
								new CaseIndexExportDto[] {
										parent,
										child });
					} else {
						resultList.add(
...
//		clinicalCourseFetch.fetch(ClinicalCourse.HEALTH_CONDITIONS);
	...
	public Predicate createCriteriaFilter(CaseCriteria caseCriteria, CriteriaBuilder cb, CriteriaQuery<?> cq, From<?, Case> from) {
		...
		if (caseCriteria.getReportingUserRole() != null) {
			filter =
					AbstractAdoService.and(cb, filter, cb.isMember(caseCriteria.getReportingUserRole(), from.join(Case.REPORTING_USER, JoinType.LEFT).get(User.USER_ROLES)));
		}
		if (caseCriteria.getDisease() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(from.get(Case.DISEASE), caseCriteria.getDisease()));
		}
		if (caseCriteria.getOutcome() != null) {
...
//					cb.and(cb.equal(responsibleRegion.get(Region.UUID), caseCriteria.getRegion().getUuid()),cb.isNull(region.get(Region.UUID)))));
		...
		if (caseCriteria.getCaseOrigin() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(from.get(Case.CASE_ORIGIN), caseCriteria.getCaseOrigin()));
		}
		if (caseCriteria.getHealthFacility() != null) {
			filter = AbstractAdoService.and(
				cb,
...
//		}
		...
		if (caseCriteria.getCreationDateFrom() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.greaterThan(from.get(Case.CREATION_DATE), DateHelper.getStartOfDay(caseCriteria.getCreationDateFrom())));
		}
		if (caseCriteria.getCreationDateTo() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.lessThan(from.get(Case.CREATION_DATE), DateHelper.getEndOfDay(caseCriteria.getCreationDateTo())));
		}
		if (caseCriteria.getQuarantineTo() != null) {
			filter = AbstractAdoService.and(
				cb,
		...
		if (caseCriteria.getPerson() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(from.join(Case.PERSON, JoinType.LEFT).get(Person.UUID), caseCriteria.getPerson().getUuid()));
		}
		if (caseCriteria.getMustHaveNoGeoCoordinates() != null && caseCriteria.getMustHaveNoGeoCoordinates() == true) {
		...
		if (caseCriteria.getMustHaveCaseManagementData() != null && caseCriteria.getMustHaveCaseManagementData() == true) {
			...
			clinicalVisitSubquery.select(clinicalVisitRoot)
				.where(cb.equal(clinicalVisitRoot.get(ClinicalVisit.CLINICAL_COURSE), from.get(Case.CLINICAL_COURSE)));
			filter = AbstractAdoService.and(cb, filter, cb.or(cb.exists(prescriptionSubquery), cb.exists(treatmentSubquery), cb.exists(clinicalVisitSubquery)));
		}
		if (Boolean.TRUE.equals(caseCriteria.getWithoutResponsibleOfficer())) {
			filter = AbstractAdoService.and(cb, filter, cb.isNull(from.get(Case.SURVEILLANCE_OFFICER)));
		}
		if (caseCriteria.getRelevanceStatus() != null) {
			if (caseCriteria.getRelevanceStatus() == EntityRelevanceStatus.ACTIVE) {
				filter = AbstractAdoService.and(cb, filter, cb.or(cb.equal(from.get(Case.ARCHIVED), false), cb.isNull(from.get(Case.ARCHIVED))));
			} else if (caseCriteria.getRelevanceStatus() == EntityRelevanceStatus.ARCHIVED) {
				filter = AbstractAdoService.and(cb, filter, cb.equal(from.get(Case.ARCHIVED), true));
			}
		}
		if (caseCriteria.getDeleted() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(from.get(Case.DELETED), caseCriteria.getDeleted()));
		}
		if (caseCriteria.getNameUuidEpidNumberLike() != null) {
			...
			for (int i = 0; i < textFilters.length; i++) {
				...
				if (!DataHelper.isNullOrEmpty(textFilter)) {
					...
					filter = AbstractAdoService.and(cb, filter, likeFilters);
				}
			}
		}
		if (caseCriteria.getReportingUserLike() != null) {
	...
	public Predicate createUserFilter(CriteriaBuilder cb, CriteriaQuery cq, From<?, Case> casePath, CaseUserFilterCriteria userFilterCriteria) {
		...
		final JurisdictionLevel jurisdictionLevel = currentUser.getJurisdictionLevel();
		//if (jurisdictionLevel != JurisdictionLevel.NATION && !currentUser.hasAnyUserRole(DefaultUserRole.REST_USER)) {
		if (jurisdictionLevel != JurisdictionLevel.NATION ) {
			...
			case REGION:
				final Region region = currentUser.getRegion();
				if (region != null) {
					filter = AbstractAdoService.or(cb, filter, cb.equal(casePath.get(Case.REGION), region));
				}
				break;
			case DISTRICT:
				final District district = currentUser.getDistrict();
				if (district != null) {
					filter = AbstractAdoService.or(cb, filter, cb.equal(casePath.get(Case.DISTRICT), district));
				}
				break;
			case HEALTH_FACILITY:
				final Facility healthFacility = currentUser.getHealthFacility();
				if (healthFacility != null) {
					filter = AbstractAdoService.or(cb, filter, cb.equal(casePath.get(Case.HEALTH_FACILITY), healthFacility));
				}
				break;
			case COMMUNITY:
				final Community community = currentUser.getCommunity();
				if (community != null) {
					filter = AbstractAdoService.or(cb, filter, cb.equal(casePath.get(Case.COMMUNITY), community));
				}
				break;
			case POINT_OF_ENTRY:
				final PointOfEntry pointOfEntry = currentUser.getPointOfEntry();
				if (pointOfEntry != null) {
					filter = AbstractAdoService.or(cb, filter, cb.equal(casePath.get(Case.POINT_OF_ENTRY), pointOfEntry));
				}
				break;
			case LABORATORY:
				...
				filter = AbstractAdoService.or(cb, filter, cb.in(casePath.get(Case.ID)).value(sampleCaseSubquery));
				break;
			default:
			}
			...
			if (userFilterCriteria == null || !userFilterCriteria.isExcludeCasesFromContacts()) {
				...
				filter = AbstractAdoService.or(cb, filter, cb.in(casePath.get(Case.ID)).value(contactCaseSubquery));
			}
			...
			if ((userFilterCriteria == null || !userFilterCriteria.isExcludeSharedCases())
				&& !featureConfigurationFacade.isFeatureDisabled(FeatureType.NATIONAL_CASE_SHARING)) {
				filter = AbstractAdoService.or(cb, filter, cb.isTrue(casePath.get(Case.SHARED_TO_COUNTRY)));
			}
		}

		// only show cases of a specific disease if a limited disease is set
		if (currentUser.getLimitedDisease() != null) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(casePath.get(Case.DISEASE), currentUser.getLimitedDisease()));
		}

		// port health users can only see port health cases
		if (UserRole.isPortHealthUser(currentUser.getUserRoles())) {
			filter = AbstractAdoService.and(cb, filter, cb.equal(casePath.get(Case.CASE_ORIGIN), CaseOrigin.POINT_OF_ENTRY));
		}

		filter = AbstractAdoService.or(cb, filter, filterResponsible);
...
//		}
	...
	private List<Case> getBatchedQueryResults(CriteriaBuilder cb, CriteriaQuery<Case> cq, Root<Case> from, Integer batchSize) {
		return null;
	}
