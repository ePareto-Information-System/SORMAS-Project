...
public class BaseAdoService<ADO extends AbstractDomainObject> implements AdoService<ADO> {
	...
	public List<Long> getIdsByReferenceDtos(Collection<? extends ReferenceDto> list) {

		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<Long> cq = cb.createQuery(Long.class);
		Root<ADO> from = cq.from(getElementClass());

		cq.where(from.get(AbstractDomainObject.UUID).in(list.stream().map(ReferenceDto::getUuid).collect(Collectors.toList())));
		//cq.where(from.get(AbstractDomainObject.UUID).in(list.stream().collect(Collectors.toList())));

		cq.select(from.get(AbstractDomainObject.ID));
		return em.createQuery(cq).getResultList();
	}
	
	public List<Long> getIdsByReferenceDtos(List<DefaultUserRole> list) {
