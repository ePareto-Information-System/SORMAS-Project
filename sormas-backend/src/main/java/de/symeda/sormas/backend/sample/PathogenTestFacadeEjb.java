...
public class PathogenTestFacadeEjb implements PathogenTestFacade {
	...
	public void validate(PathogenTestDto pathogenTest) throws ValidationRuntimeException {
		...
		if (pathogenTest.getTestType() == null && !usesVhfSpecificTestResults(pathogenTest.getTestedDisease())) {
		...
		if (pathogenTest.getTestResult() == null && !usesVhfSpecificTestResults(pathogenTest.getTestedDisease())) {
			throw new ValidationRuntimeException(
				I18nProperties.getValidationError(
					Validations.required,
	...
	static boolean usesVhfSpecificTestResults(Disease disease) {
		return disease == Disease.UNSPECIFIED_VHF || Disease.AHF_DISEASES.contains(disease);
	}
