...
public class DiseaseFacadeEjb implements DiseaseFacade {
	...
	public List<DiseaseBurdenDto> getDiseaseBurdenForDashboard(
		...
		Map<Disease, Long> events =
			eventFacade.getEventCountByDisease(new EventCriteria().region(regionRef).district(districtRef).eventDateBetween(from, to));
...
//		return null;
	...
	public DiseaseBurdenDto getDiseaseGridForDashboard(
		...
		//Get the region
		RegionDto regionDto = FacadeProvider.getRegionFacade().getByUuid(regionRef.getUuid());
